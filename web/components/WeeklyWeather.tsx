...
export function WeeklyWeather() {
  ...
  return (
    <div className="bg-surface-container-low rounded-3xl overflow-hidden border border-outline-variant/15 relative">
      <div className="px-6 py-4 sm:px-8 sm:py-6 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-surface-container/50 gap-4 border-b border-outline-variant/10">
        <div className="flex items-center gap-3">
          <span className="material-symbols-outlined text-primary" translate="no">location_on</span>
          <h3
            className="text-lg sm:text-xl font-bold tracking-tight text-on-surface"
          ...
          <div className="relative group flex items-center ml-1">
             <span className="material-symbols-outlined text-on-surface-variant text-[16px] cursor-help transition-colors hover:text-primary" translate="no">info</span>
             <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-64 p-3 bg-surface-container-high border border-outline-variant/40 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all text-xs text-on-surface-variant z-50 text-center leading-relaxed">
               ...
               <div className="absolute left-1/2 -top-2 -translate-x-1/2 border-x-8 border-x-transparent border-b-8 border-b-surface-container-high"></div>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          {maxPages > 1 && (
            <div className="flex items-center gap-1 bg-surface-container-high border border-outline-variant/30 rounded-xl p-1 shadow-inner">
              <button 
                ...
                disabled={weekPage === 0}
                className={`p-1.5 rounded-lg flex items-center justify-center transition-colors ${weekPage === 0 ? 'text-on-surface-variant/30 cursor-not-allowed' : 'text-on-surface hover:bg-outline-variant/40 cursor-pointer'}`}
                title="Previous week"
              >
              ...
              <button 
                ...
                className={`p-1.5 rounded-lg flex items-center justify-center transition-colors ${weekPage >= maxPages - 1 ? 'text-on-surface-variant/30 cursor-not-allowed' : 'text-on-surface hover:bg-outline-variant/40 cursor-pointer'}`}
                title="Next week"
              >
          ...
          <div className="relative w-full sm:w-64 z-10" ref={dropdownRef}>
            <div className="flex items-center bg-surface-container-high border border-outline-variant/30 rounded-xl px-3 py-2 shadow-inner focus-within:border-primary/50 transition-colors">
              <span className="material-symbols-outlined text-on-surface-variant text-sm mr-2" translate="no">search</span>
              <input
                type="text"
          ...
          {showDropdown && search.trim().length > 1 && (
            <div className="absolute top-full left-0 w-full mt-2 bg-surface-container-high border border-outline-variant/40 rounded-xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto z-50">
              {isSearching ? (
                <div className="px-4 py-3 text-sm text-on-surface-variant flex items-center justify-center">
                   <div className="animate-spin w-4 h-4 rounded-full border-2 border-on-surface-variant/20 border-t-on-surface-variant mr-2"></div>
                   Searching...
                </div>
              ) : results.length > 0 ? (
                <ul className="divide-y divide-outline-variant/10">
                  {results.map((city) => (
                    <li
                      key={city.id}
                      className="px-4 py-3 text-sm text-on-surface hover:bg-primary/10 cursor-pointer transition-colors flex flex-col"
      ...
      <div className="px-6 sm:px-8 h-full">
        {isLoadingWeather || days.length === 0 ? (
          <div className="h-40 flex items-center justify-center">
             <div className="animate-spin w-8 h-8 rounded-full border-2 border-primary/20 border-t-primary"></div>
          </div>
        ) : (
          <div className="flex overflow-x-auto gap-4 sm:gap-0 sm:grid sm:grid-cols-7 sm:divide-x divide-outline-variant/15 min-w-max sm:min-w-0 h-full">
            {days.slice(weekPage * 7, (weekPage + 1) * 7).map((day) => {
              ...
              return (
                <div
                  key={day.date}
                  className={`flex flex-col items-center justify-center py-6 px-4 sm:px-6 w-32 sm:w-auto shrink-0 transition-colors ${isToday ? "bg-primary/[0.06] shadow-inner" : ""}`}
                ...
                >
                  <div className="flex items-center justify-center gap-1.5 mb-4">
                    ...
                    <span className={`text-[11px] font-bold tracking-[0.2em] uppercase ${isToday ? "text-primary" : "text-on-surface-variant"}`}>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xl font-bold text-on-surface" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
                  ...
                  {/* Precipitation */}
                  <div className={`flex items-center gap-1 mt-2 transition-opacity ${day.precipProb > 0 ? 'text-blue-400 opacity-90' : 'text-on-surface-variant opacity-20'}`}>
