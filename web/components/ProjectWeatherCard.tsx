...
async function fetchFromNWS(lat: number, lon: number): Promise<WeatherDay[]> {
  ...
  for (let i = 0; i < periods.length; i++) {
    const p = periods[i];
    if (!p.isDaytime) continue; // Start from daytime periods

    const dateStr = p.startTime.slice(0, 10); // YYYY-MM-DD
    const nightP = periods[i + 1]; // Corresponding night period
                ...
                >
                  ...
                  <div
                    className={`flex items-center gap-1 mt-2 transition-opacity ${
                      day.precipProb > 0 ? "text-blue-400 opacity-90" : "text-[#ababa8] opacity-20"
                    }`}
                  >
                    <span
